            ...
            <div style="border: 1px solid #D3D3D3; background-color: #FFFFFF; margin-left: 20%; margin-right: 20%;">
                <div class="row">
                    ...
                    <div class="col-md-8">
                        ...
                        <td>
                            <br />
                            <asp:Button class="btn btn-primary rounded" Text="More Details" ID="Button1" runat="server" PostBackUrl='<%# string.Format("~/house.aspx?ID={0}&hname={1}&housetype={2}&address={3}&postcode={4}&city={5}&description={6}&accommodation={7}&rentprice={8}&duration={9}&status={10}", HttpUtility.UrlEncode(Eval("ID").ToString()),HttpUtility.UrlEncode(Eval("hname").ToString()),HttpUtility.UrlEncode(Eval("housetype").ToString()),HttpUtility.UrlEncode(Eval("address").ToString()),HttpUtility.UrlEncode(Eval("postcode").ToString()),HttpUtility.UrlEncode(Eval("city").ToString()),HttpUtility.UrlEncode(Eval("description").ToString()),HttpUtility.UrlEncode(Eval("accommodation").ToString()),HttpUtility.UrlEncode(Eval("rentprice").ToString()),HttpUtility.UrlEncode(Eval("duration").ToString()),HttpUtility.UrlEncode(Eval("status").ToString())) %>'></asp:Button>
                        </td>
                        </tr>
                    </div>
                </div>
